...
function renderAssetsTable() {
  ...
  tbody.innerHTML = state.assets.map(a => {
    ...
    return `<tr><td><strong>${a.name}</strong></td><td>${a.opened}</td><td><input type="number" value="${a.balance}" data-asset-balance="${a.id}"></td><td>${a.allocation}</td><td>${pct.format(share)}</td><td>${a.ytd.toFixed(1)} %</td><td>${a.tax}</td><td><canvas class="sparkline" id="spark-${a.id}" width="98" height="28"></canvas></td></tr>`;
  }).join('');
...
function baseOptions() { return { responsive: true, maintainAspectRatio: false, plugins: { legend: { labels: { color: cssVar('--muted') } } }, scales: { x: { ticks: { color: cssVar('--muted') }, grid: { color: 'rgba(148,163,184,.08)' } }, y: { ticks: { color: cssVar('--muted') }, grid: { color: 'rgba(148,163,184,.08)' } } } }; }
function renderAllocationChart() {
  ...
  charts.allocationChart = new Chart(el, { type: 'doughnut', data: { labels: groups, datasets: [{ data: groups.map(g => state.assets.filter(a => a.category === g).reduce((s,a)=>s+a.balance,0)), borderWidth: 0 }] }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'bottom', labels: { color: cssVar('--muted') } } } } });
}
